...
def rescan():
    ...
    if s is not None and t is not None:
        ...
        ori_x = int((t.left + t.width * 2.8 - s.left) / 2)
        ...
        ori_x = int((t_full.left + t_full.width * 1.55 - s_full.left) / 2)
...
def on_click(x, y, button, pressed):
    predict_fast('yuji_full.png|yuji.png')
    if not pressed:
        return False


def on_click_single(x, y, button, pressed):
    enemy_pos = enemy_locate('yuji_full.png|yuji.png')  # mark.png|mark_full.png|suan.png|suan_full.png|
...
def aim_single():
    find()
    with pynput.mouse.Listener(on_click=on_click_single) as listener:
        listener.join()
    print('aimed')


def predict(args):
    global aim_x, aim_y, enemy_pos_last, enemy_pos_now
    enemy_pos_pre = [0, 0]
...
def predict_fast(args):
    ...
    if enemy_pos_now is not None and enemy_pos_last is not None:
        aim_x = (enemy_pos_now[0] * (1 + momentum) - enemy_pos_last[0] * momentum - me_x) * YAxisRatio * Radius + left + ori_x
        aim_y = (enemy_pos_now[1] * (1 + momentum) - enemy_pos_last[1] * momentum - me_y) * Radius + top + ori_y
